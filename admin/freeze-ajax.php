...
add_action( 'wp_ajax_eos_freeze_rename_folder','eos_freeze_rename_folder' );
//Rename plugins Folder
function eos_freeze_rename_folder(){
  if(
    !current_user_can( 'manage_options' )
    || !isset( $_POST['nonce'] )
    || !isset( $_POST['new_name'] )
    || !wp_verify_nonce( sanitize_text_field( $_POST['nonce']  ),'eos_freeze_rename_nonce' )
  ){
    die();
    exit;
  }
  ...
  $writeAccess = false;
    ...
    $new_folder_name = sanitize_file_name( $_POST['new_name'] );
  	$new_line_dir = "define('WP_PLUGIN_DIR','".WP_CONTENT_DIR."/".$new_folder_name."'); // Added by Freeze\r\n";
  	$new_line_url = "define('WP_PLUGIN_URL','".content_url()."/".$new_folder_name."'); // Added by Freeze\r\n";
  	$new_line_name = "define('PLUGINDIR','wp-content/".$new_folder_name."'); // Added by Freeze\r\n";
    $unsetIdx = array();
    $n = 0;
    ...
    $handle = @fopen( $config_file_path, 'w' );
  	...
  	@fclose( $handle );
    $plugins_path = dirname( EOS_FREEZE_PLUGIN_DIR );
